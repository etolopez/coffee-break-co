...
@Injectable()
export class SellerCoffeeService {
  private readonly logger = new Logger(SellerCoffeeService.name);
   ...
   */
  private async getSellerIdFromUserId(userId: string): Promise<string | null> {
    try {
   ...
   */
  async getMyCoffees(userId: string): Promise<CoffeeEntry[]> {
    ...
    if (!sellerId) {
      ...
      throw new NotFoundException('Seller profile not found. Please create a seller profile first.');
    }
   ...
   */
  async getCoffeeById(coffeeId: string, userId: string): Promise<CoffeeEntry> {
    const sellerId = await this.getSellerIdFromUserId(userId);
    if (!sellerId) {
    ...
    if (coffee.sellerId !== sellerId) {
      throw new ForbiddenException('You do not have permission to access this coffee');
    }
   ...
   * Create a new coffee
   */
  async createCoffee(coffeeData: any, userId: string): Promise<CoffeeEntry> {
    const sellerId = await this.getSellerIdFromUserId(userId);
    if (!sellerId) {
      throw new NotFoundException('Seller profile not found. Please create a seller profile first.');
    }
    ...
    if (!coffeeData.coffeeName || !coffeeData.origin) {
      throw new BadRequestException('coffeeName and origin are required');
    }
    ...
    if (existingCoffee) {
      throw new BadRequestException('A coffee with this slug already exists');
    }
  ...
  async updateCoffee(
    ...
    if (existingCoffee.sellerId !== sellerId) {
      throw new ForbiddenException('You do not have permission to update this coffee');
    }

    // Update coffee
    const updateData: any = {};
    if (coffeeData.coffeeName !== undefined) updateData.coffeeName = coffeeData.coffeeName;
    if (coffeeData.origin !== undefined) updateData.origin = coffeeData.origin;
    if (coffeeData.description !== undefined) updateData.description = coffeeData.description;
    if (coffeeData.farm !== undefined) updateData.farm = coffeeData.farm;
    if (coffeeData.farmer !== undefined) updateData.farmer = coffeeData.farmer;
    if (coffeeData.altitude !== undefined) updateData.altitude = coffeeData.altitude;
    if (coffeeData.variety !== undefined) updateData.variety = coffeeData.variety;
    if (coffeeData.process !== undefined) updateData.process = coffeeData.process;
    if (coffeeData.harvestYear !== undefined) updateData.harvestYear = coffeeData.harvestYear;
    if (coffeeData.roastLevel !== undefined) updateData.roastLevel = coffeeData.roastLevel;
    if (coffeeData.flavorNotes !== undefined) updateData.flavorNotes = coffeeData.flavorNotes;
    if (coffeeData.cuppingScore !== undefined) updateData.cuppingScore = coffeeData.cuppingScore;
    if (coffeeData.notes !== undefined) updateData.notes = coffeeData.notes;
    if (coffeeData.price !== undefined) updateData.price = coffeeData.price;
    if (coffeeData.currency !== undefined) updateData.currency = coffeeData.currency;
    if (coffeeData.weight !== undefined) updateData.weight = coffeeData.weight;
    if (coffeeData.region !== undefined) updateData.region = coffeeData.region;
    if (coffeeData.coordinates?.lat !== undefined) updateData.coordinatesLat = coffeeData.coordinates.lat;
    if (coffeeData.coordinates?.lng !== undefined) updateData.coordinatesLng = coffeeData.coordinates.lng;
    if (coffeeData.certifications !== undefined) updateData.certifications = coffeeData.certifications;
    if (coffeeData.environmentalPractices !== undefined)
      updateData.environmentalPractices = coffeeData.environmentalPractices;
    if (coffeeData.available !== undefined) updateData.available = coffeeData.available;
   ...
   */
  async deleteCoffee(coffeeId: string, userId: string): Promise<void> {
    const sellerId = await this.getSellerIdFromUserId(userId);
    if (!sellerId) {
      throw new NotFoundException('Seller profile not found.');
    }
    ...
    if (existingCoffee.sellerId !== sellerId) {
      throw new ForbiddenException('You do not have permission to delete this coffee');
    }
